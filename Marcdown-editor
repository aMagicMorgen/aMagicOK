Да, существуют визуальные редакторы Markdown (MD), которые позволяют редактировать файлы в режиме WYSIWYG (как HTML-страницу) и сохранять изменения в исходном MD-формате. Вот несколько подходящих вариантов из представленных в результатах поиска:
